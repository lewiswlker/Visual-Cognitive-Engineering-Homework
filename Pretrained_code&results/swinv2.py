...
def load_model(device):
    model = Swinv2ForImageClassification.from_pretrained(
        "/root/swin/model/",  # 指向包含 config.json 和 pytorch_model.bin 的目录
        ignore_mismatched_sizes=True
    )
    model.classifier = torch.nn.Linear(model.classifier.in_features, 100)  # CIFAR-100有100个类别
...
def main():
    ...
    train_model(model, train_loader, test_loader, criterion, optimizer, num_epochs=20, eval_interval=1,
